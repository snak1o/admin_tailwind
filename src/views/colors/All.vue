    ...
    <div class="grid grid-cols-4 mt-10 gap-10">
      <div v-for="(color, index) in colors"  :key="color.id" class="px-3 pt-3 pb-2 rounded-md border border-gray-400 relative" :class="color.hex === '#ffffff' ? 'text-black' : 'text-white'" :style="{backgroundColor: color.hex}">
        ...
        <OptionsPopUp ref="optionsPopUp" class="top-10 right-12 border">
          <ul>
            <li class="text-black py-1 px-2 border-b hover:bg-gray-100 cursor-pointer transition" @click="addToAllProducts(color.id)">
              Добавить к товарам
            </li>
            <li class="text-black py-1 px-2 border-b text-red-500 hover:bg-gray-100 cursor-pointer transition" @click="removeFromAllProducts(color.id)">
...
export default {
  ...
  methods: {
    ...
    async removeColor(id) {
      const res = await this.$axios.delete('/api/v1/colors/' + id)
      if (res.status === 200) {
